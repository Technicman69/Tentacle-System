...
# move nodes
execute as @e[type=marker,tag=tent.node] if score $current tent.uuid = @s tent.uuid if score @s tent.node_id = $next_node tent.node_id run function tech:tentacle/_update_loop with storage tech:tent update
